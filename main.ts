import { FreeHtmlParser } from "./functions";
const fs = require('fs');
...
switch(args[2]){
    case '--about' || '-about':
        ...
        break;
    case '--help' || '-help' || '-?' || '--?':
        console.log("    --about About GFP\n    [FHT File Path] [Out Path] Build HTML file\n");
        break;
    case undefined:
        fs.access(`./fhtconfig.json`, fs.constants.F_OK, (err:string) =>{
            if(err){
        ...
        break;
    default:
        fs.access(`${args[2]}`, fs.constants.F_OK, (err:string) => {
            if(err){
                ...
                if(data){
                    const _parse = new FreeHtmlParser(data);
                    const _result:string = _parse.parse();
                    if(args[3] != null){
